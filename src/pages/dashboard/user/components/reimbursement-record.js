...
function ReimbursementRecord() {
  ...
  // const pendingCount = data.filter((item) => item.status === "wait").length;
  // const reviewedCount = data.filter((item) => item.status === "approved" || item.status === "rejected").length;
  ...
  useEffect(() => {
    ...
    setCurrentPage(1); // 搜索条件变化时重置到第一页
  }, [searchParams, data]);
